...
echo "$($PSQL "SELECT t.name FROM teams t INNER JOIN games g ON t.team_id=g.winner_id OR t.team_id=g.opponent_id WHERE year=2014 AND round='Eighth-Final' ORDER BY t.name")"
...
echo "$($PSQL "SELECT g.year, t.name FROM teams t INNER JOIN games g ON t.team_id=g.winner_id WHERE g.round='Final' ORDER BY year")"
